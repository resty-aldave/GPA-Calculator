# SWU GPA Calculator 🎓

## ✨ Features

- **Dual Modes for Flexibility**:
    - **Detailed Mode**: Input individual grades for every assessment in Prelims, Midterms, and Finals. We handle the averaging for you.
    - **Quick Mode**: Already know your periodic averages? Enter them directly for an instant result.
  
- **Smart Logic**:
    - **Real-time Calculation**: See your Semester Average and GPA update instantly as you type.
    - **Automatic Validation**: Ensures all inputs are valid grades (0-100), preventing accidental errors.
    - **SWU Grading System**: Automatically converts your Semester Average into the standard GPA point scale (e.g., 95 → 1.20).

## 🚀 How to Use

1.  **Choose Your Mode**:
    - Use the toggle at the top to switch between *Detailed Calculator* and *Quick Average*.
2.  **Enter Your Grades**:
    - Type in your scores. In Detailed Mode, you can add as many grade entries as needed for each period.
3.  **Get Your Result**:
    - Click **Calculate GPA** to reveal your Semester Average and final GPA rating.
